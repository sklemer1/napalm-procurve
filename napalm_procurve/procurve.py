...
class ProcurveDriver(NetworkDriver):
    ...
    def _get_interface_map(self):
        ...
        if len(self.interface_map) < 1 or "pytest" in sys.modules:
            self.interface_map = {
                v: k for k, v in self._walkMIB_values("ifName").items()
            }
    ...
    def get_lldp_neighbors(self):
        ...
        for lldp_entry in split_output.splitlines():
            ...
            entry = {"port": str(remote_port), "hostname": str(device_id)}
    ...
    def get_environment(self):
        ...
        try:
            environment["cpu"][0]["%usage"] = float(show_cpu_1m.split("/")[0].strip())
        except KeyError:
            pass
            ...
            for line in output.splitlines():
                try:
                    ...
                    ping_dict["success"]["results"].append(
                        {
                            "ip_address": str(ping_data.group(1)),
                            "rtt": float(ping_data.group(3)),
                        }
                    )
                    ...
                    if line in ("Target did not respond.", "Request timed out."):
                        ...
                        ping_dict["success"]["packet_loss"] = ping_dict["success"][
                            "probes_sent"
                        ] - int(ping_data.group(2))
                        ...
                        ping_dict["success"].update(
                            {
                                "rtt_min": float(ping_data.group(1)),
    ...
    def get_ntp_servers(self):
        ...
        try:
            split_sntp = re.split(r"^  -----.*$", output, flags=re.M)[1].strip()
